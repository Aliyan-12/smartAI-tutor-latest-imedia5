...
### Fast text streaming + TTS (useSessionChannel) — DECOUPLED
Text and audio are **two independent streams** so text never waits on Kokoro (GPT-style speed):
- Backend `stream_segment()` sends a text `{type:"segment", seq, turn_id, text}` frame **immediately**, then (if `tts`) spawns a background `_tts_segment` task that ships a separate `{type:"segment_audio", seq, turn_id, audio_b64, duration_ms}` frame — exactly one per seq (a null clip for short/failed text so the client's queue never stalls). `turn_end` is NOT gated on TTS.
- Client runs a **text pump** (reveals each segment at a reading cadence the instant it arrives) and an independent **audio pump** (buffers `segment_audio` by seq, plays strictly in `nextAudioSeq` order). Frames are filtered by `turn_id` so leftover audio from a prior turn is dropped. Mute pauses the in-flight clip; text still completes.
- The client sends `tts: ttsEnabledRef.current` per message; `/chat` backend overrides by mode (text turn = no TTS, voice turn = TTS). Voice = Kokoro **af_heart** (warm) at speed 0.95; `_prep_tts_text` keeps commas/dashes/ellipses + turns breaks into pauses (Kokoro has no style prompt).

### "Thinking" strip (replaces voice fillers)
Voice fillers (`seed_voice_fillers.py`, neutral bridge clips) are **removed**. Each turn now shows a Claude-style **thinking strip**: one-line steps for what the tutor did — tool labels from `_THINKING_LABELS` (e.g. "🧩 Setting up a puzzle", "✓ Checking your answer") + a brief best-effort Gemini **thought summary** (`include_thoughts` in `llm_service`; thought parts → `[THINK:…]` in `gemini_service.stream_response_async`). Steps stream live as `{type:"thinking", text}` and are **persisted** as a `role="thinking"` chat message (mirrors `role="event"`; filtered out of LLM history by `build_context`), so they reappear after refresh. Rendered by `ChatWindow`'s `ThinkingStrip`.
...
`_run_turn` computes the per-turn group set via `select_tool_groups(event_kind, intent_text, has_slides, end_allowed, quiz_phase, closing_stage)` — **STATE-DRIVEN first, then query-driven**. State sets the base the current stage needs: the **core in-lesson view tools are ALWAYS bound** (`puzzles` always + `teaching` when there are slides) so the model can always switch slides↔puzzles and `show_puzzle` is never missing (a missing `show_puzzle` used to silently no-op → the model hallucinated "look at the puzzle"); `assessment` whenever the **quiz window is open** (`quiz_phase`) regardless of keywords; `lifecycle` (end + report) whenever `end_allowed`, the lesson is in its **closing stage** (last ~15%), or the student signals ending. Keyword intent only ADDS extra groups on top. `mastery` rides along with puzzles/assessment. Passed to `gemini_service.stream_response_async(..., tool_groups=…)`; if the model calls a tool that isn't bound this turn, `gemini_service` feeds back a `tool_unavailable` ToolMessage (no silent drop) so it can't claim the action happened. The **LESSON STATE anchor** advertises "AVAILABLE ACTIONS THIS TURN", the **valid puzzle_ids + label-diagram keys** (so the model never guesses a template), and a single **`⚡ DO NOW`** imperative (advance slide / set quiz / wrap up / end) driven by the clock — so the model acts on lesson state instead of waiting to be asked. Slide moves clear any on-screen puzzle (slides↔puzzles are mutually-exclusive views) on both backend (`puzzle_state`) and frontend.
...
- Templates (Maths + Science): SVG — `fraction_bar`, `pie_fraction`, `number_line`, `place_value`, `array_grid`, `shape_count`, `area_grid`, `clock`, `angle`, `coordinate_grid`, `bar_chart`, `balance_scales`, `particle_state`, `formula_triangle`; react-konva — `build_fraction`, `label_diagram`, `states_of_matter`, `food_chain_order`. Every template is tagged with a **category** (`labelling`, `matching`, `recognition`, `sorting`, `sequencing`, `counting`, `fractions`, `number`, `geometry`, `data`, `algebra`) shown as a chip; `list_available_puzzles` reports it so the model picks by kind.
- **Image puzzles (real curriculum images).** `identify_image` (show a real topic image → name it; reuses the `choice` flow) and `match_image` (match several topic images ↔ names; tap-to-pair). Both pull genuine images from a **topic-image catalog** (`rh_topic_images`) built by `services/topic_image_service.py` + `resource_sync_service.sync_topic_images()` (resolves a Wikipedia lead image per `rh_topics` row, scoped by `rh_availability` to KS/Year; cached as stable URLs — never live-searched at play time). `show_puzzle` injects catalog rows (scoped to the lesson's subject/KS/topic via `ToolContext.topic_title`) as `params["_catalog"]`; too few images → `no_catalog_images` → model falls back to a typed question. Seed/refresh: `python -m app.seed_topic_images` (also on the curriculum sync schedule + `POST /api/curriculum/sync?target=topic_images`).
- New `session_tools.py` tools: `list_available_puzzles()`, `show_puzzle(puzzle_id, params)`, `clear_puzzle()`. `show_puzzle` returns `{action:"show_puzzle", ...}` → WS `{type:"tool", tool:"show_puzzle"}` (same pipeline as slides). Attempt returns via `puzzle_result` → `_handle_puzzle_result` → AI praises/advances or hints. System prompt has a **VISUAL PUZZLES** block (now "choose by category").
- Frontend `components/PuzzlePlayer.tsx` + `components/puzzles/*` render by `render` key (incl. `IdentifyImage`, `MatchImage`, `CategoryChip`); `useSessionChannel.sendPuzzleResult(...)` mirrors `sendQuizResult`; `SessionPage` shows the puzzle in the Learn panel (overlays the slide while active).
