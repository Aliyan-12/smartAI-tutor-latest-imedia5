...
from utils.simple_auth import create_session_token, get_configured_auth_username
...
def _build_presentation_export_url(presentation_id: uuid.UUID) -> tuple[str, str | None]:
    ...
    # Pass an internal session token so pdf-maker can authenticate FastAPI calls
    auth_username = get_configured_auth_username()
    if auth_username:
        internal_token = create_session_token(auth_username)
        params["exportSession"] = internal_token
