"""
jobs/sync_service.py — the Resource Hub client + the background sync jobs, in one place.

Client  ResourceHubClient — async client for the external Resource Hub API (curriculum +
                            resources), used only by the sync jobs below.
...
(Merged from the former resource_hub_client.py + resource_sync_service.py.)
"""
import asyncio
import hashlib
import logging
...
# Base URL (`settings.resourcehub_api_url`) already includes the `/api` prefix; this client
# appends `/v1/...`. Auth is a Bearer token (the hub also accepts `?api_key=`, sent as a
# fallback). Usage: `async with ResourceHubClient() as hub: await hub.get_keystages()`.
